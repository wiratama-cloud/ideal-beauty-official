...
export async function main() {
  ...
  const createdProducts = await Promise.all(
    productsData.map(async (productInfo) => {
      ...
      console.log(`Upserted product: ${createdProduct.name} (${updatedVariants.length} variants)`);
      return {
        ...createdProduct,
  ...
  // Log initial R&D / COGS expense entries into Ledger if not present (in batch)
  ...
  const existingProductIdsWithLedger = new Set(existingLedgers.map((l) => l.productId));

  const missingLedgers = createdProducts
    .filter((p) => !existingProductIdsWithLedger.has(p.id))
    .map((p) => ({
      type: 'EXPENSE' as const,
      amount: 5000000.0,
      description: `R&D and Haute Couture Design for ${p.name}`,
      expenseCategory: 'DESIGN_RND' as const,
      productId: p.id,
    }));
  ...
  // Create initial InventoryTransaction entries for variant stock allocations if not present (in batch)
  const allVariants = createdProducts.flatMap((p) => p.variants);
  const allVariantIds = allVariants.map((v) => v.id);

  const existingTxs = await prisma.inventoryTransaction.findMany({
    where: {
  ...
  const existingVariantIdsWithTx = new Set(existingTxs.map((t) => t.variantId));

  const missingTxs = allVariants
    .filter((v) => !existingVariantIdsWithTx.has(v.id))
    .map((v) => ({
      variantId: v.id,
      type: 'ADD' as const,
